...
public class FilmeServiceImpl implements FilmeService {
    ...
    public List<FilmeDto> listarFilmes() {
        ...
        return filmes.stream().map(f -> new ModelMapper().map(f, FilmeDto.class))
        .collect(Collectors.toList());
    }

    @Override
    public FilmeDto criarFilme(FilmeDto filme){
        Filme filmeParaGrava = new ModelMapper().map(filme, Filme.class);
        filmeParaGrava = repo.save(filmeParaGrava);

        return new ModelMapper().map(filmeParaGrava, FilmeDto.class);
    }
